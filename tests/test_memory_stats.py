...
def test_extract_helpers_cover_flat_nested_and_invalid_entries():
    ...
    assert stats_mod._extract_verdict({"transcript": {"verdict": {"decision": "Concern"}}}) == "concern"
    assert stats_mod._extract_verdict("bad") is None

    assert stats_mod._extract_divergence({"divergence_analysis": {"concern": ["risk"]}}) == {
        "concern": ["risk"]
    }
    assert stats_mod._extract_divergence({"transcript": {"divergence_analysis": {"object": ["proof"]}}}) == {
        "object": ["proof"]
    }
    ...
    assert stats_mod._extract_coherence({"transcript": {"coherence": {"min_confidence": 0.3}}}) == 0.3
...
def test_count_by_verdict_and_most_common_divergence():
    entries = [
        ...
        {"transcript": {"verdict": {"decision": "approve"}, "divergence_analysis": {"concern": ["risk"]}}},
        {"verdict": "concern"},
    ]
