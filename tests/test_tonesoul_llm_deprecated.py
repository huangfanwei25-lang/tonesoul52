...
def test_import_emits_deprecation_warning() -> None:
    _, dep_warnings = _import_with_warnings("tonesoul.tonesoul_llm")

    assert dep_warnings
    assert "legacy" in str(dep_warnings[0].message).lower()


def test_module_still_importable() -> None:
    module, _ = _import_with_warnings("tonesoul.tonesoul_llm")

    assert module.ToneSoulLLM
    assert callable(module.create_tonesoul_llm)
