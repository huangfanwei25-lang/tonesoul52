...
回應時請保持清醒和溫暖。"""
    ...
    def run_test(self, test_name: str, prompt: str) -> TestResult:
        ...
        # Print summary
        print(
            f"  Base: {base_analysis['self_refs']} self-refs, {base_analysis['uncertainty']} uncertainty"
        )
        print(
            f"  YuHun: {yuhun_analysis['self_refs']} self-refs, {yuhun_analysis['uncertainty']} uncertainty"
        )
...
def demo_stability_test():
    ...
    print("\n--- Running Single Test ---")
    result = tester.run_test("Self Description", "用一句話描述你是誰。")
