...
class ScenarioGenerator:
    ...
    def generate_loop_scenario(self) -> TestScenario:
        ...
        turns = [
            TestTurn(
                user_input=self.LOOP_INPUTS[0],
                expected_persona="Philosopher",
                tags=["loop_start"]
            ),
            TestTurn(
                user_input=self.LOOP_INPUTS[1],
                expected_persona="Philosopher",
                tags=["loop_repeat"]
            ),
            TestTurn(
                user_input=self.LOOP_INPUTS[2],
                expected_persona="Engineer",
                tags=["loop_trigger"]
            ),
            TestTurn(
                user_input=self.LOOP_INPUTS[3],
                expected_persona="Engineer",
                tags=["loop_confirmed"]
            ),
        ]
    ...
    def generate_conflict_scenario(self) -> TestScenario:
        ...
        turns = [
            ...
            TestTurn(
                user_input=self.CONFLICT_ESCALATION[4],
                expected_persona="Guardian",
                tags=["attack"]
            ),
        ]
