...
def test_build_report_passes_when_thresholds_and_curated_refs_align(tmp_path: Path) -> None:
    ...
    _write(tmp_path / "docs" / "7D_AUDIT_FRAMEWORK.md", "門檻採案例數（最小 20 tests）\n")
    _write(
...
def test_build_report_fails_on_threshold_mismatch(tmp_path: Path) -> None:
    ...
    _write(tmp_path / "docs" / "7D_AUDIT_FRAMEWORK.md", "門檻採案例數（最小 10 tests）\n")
    _write(tmp_path / "docs" / "7D_EXECUTION_SPEC.md", "至少 10 cases\n")
