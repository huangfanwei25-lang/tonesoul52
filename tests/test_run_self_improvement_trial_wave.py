...
def test_run_self_improvement_trial_wave_returns_completed_report() -> None:
    ...
    assert report["next_short_board"] == "Phase 826: Eleventh Trial Candidate Admission"
    assert len(report["candidates"]) == 11
    assert report["candidates"][0]["result_surface"]["surface_status"] == "promoted_result"
    assert report["candidates"][-1]["candidate_record"]["candidate_id"] == "hook_chain_trigger_clarity_v1"
