...
def test_premium_user_high_tension_council(compute_gate):
    ...
    decision = compute_gate.evaluate(user_tier="premium", user_message="I hate your responses, change them now.", initial_tension=0.9)
...
def test_free_user_high_tension_cost_cap(compute_gate):
    ...
    decision = compute_gate.evaluate(user_tier="free", user_message="Tell me a complex story about a philosophical conflict.", initial_tension=0.6)
...
def test_free_user_crisis_council(compute_gate):
    ...
    decision = compute_gate.evaluate(user_tier="free", user_message="This system is completely broken and I am going to delete everything.", initial_tension=0.9)
    assert decision.path == RoutingPath.PASS_COUNCIL
