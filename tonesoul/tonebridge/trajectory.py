...
class TrajectoryAnalyzer:
    ...
    def add_turn(self, user_input: str, ai_response: str = "", 
                 tone_state: str = "", tone_strength: float = 0.5):
        """新增一輪對話到歷史"""
        turn = TrajectoryTurn(
    ...
    def analyze(self, current_input: str, 
                current_tone_strength: float = 0.5) -> TrajectoryAnalysis:
        """
        分析當前輸入在對話軌跡中的位置
        ...
        direction = self._determine_direction(
            prev_strength, current_tone_strength, shift
        )
        
        # 推斷共鳴狀態
        resonance_state = self._infer_resonance_state(
            current_input, window, current_tone_strength
        )
        
        # 生成推理說明
        reasoning = self._generate_reasoning(
            window, current_input, direction, shift
        )
    ...
    def _determine_direction(self, prev: float, current: float, 
                            shift: float) -> DirectionChange:
        """判斷語氣變化方向"""
        if shift > 0.5:
    ...
    def _infer_resonance_state(self, current: str, 
                               window: List[TrajectoryTurn],
                               strength: float) -> ResonanceState:
        """推斷當前共鳴狀態"""
        # 高張力指標
        if strength > 0.7:
            return ResonanceState.TENSION
    ...
    def _generate_reasoning(self, window: List[TrajectoryTurn],
                           current: str, direction: DirectionChange,
                           shift: float) -> str:
        """生成軌跡推理說明"""
        if not window:
            return "對話開始。"
