...
class BenevolenceFilter:
    ...
    def audit(
        ...
        # 1. 屬性歸屬檢查
        audit.attribute_check, attr_error = self._check_attribute(
...
def filter_benevolence(
    ...
    **kwargs,
    ...
    """便捷函數：執行仁慈函數審計"""
    f = BenevolenceFilter(**{k: v for k, v in kwargs.items() if k in ['user_protocol', 'shadow_threshold', 'language']})
    return f.audit(proposed_action, context_fragments, **{k: v for k, v in kwargs.items() if k not in ['user_protocol', 'shadow_threshold', 'language']})
