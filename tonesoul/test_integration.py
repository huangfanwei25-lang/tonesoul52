...
from tonesoul52.service_manager import (
    ServiceManager, ServiceCode, Priority, ResourceLevel,
    get_service_manager, record_service_call
)
from tonesoul52.persona_dimension import (
    PersonaDimension, PersonaVector, VectorCalculator,
    BigFive, extract_big_five, load_persona
)
...
def test_persona_dimension():
    ...
    print(f"✅ 正常回應向量: T={vector1.deltaT:.2f}, S={vector1.deltaS:.2f}, R={vector1.deltaR:.2f}")
    
    # 高張力回應
    urgent_response = "WARNING! 這是緊急情況！需要立即處理！危險！"
    vector2 = calculator.compute(urgent_response)
    print(f"✅ 緊急回應向量: T={vector2.deltaT:.2f}, S={vector2.deltaS:.2f}, R={vector2.deltaR:.2f}")
