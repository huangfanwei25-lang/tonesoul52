    ...
    if concerns:
        ...
        if actionable_concerns and len(refinement_concerns) >= 2:
            corr_sources = [
                ...
                for v in actionable_concerns
            ]
            factors.append(
                f"Actionable substantive concerns from: {', '.join(corr_sources)}, "
                f"corroborated by {len(refinement_concerns)} total refinement concerns"
            )
            alternatives.append(
                AlternativePath(
                    verdict_candidate="approve",
                    ...
                    rejected_because=(
                        "Actionable substantive concern corroborated by another perspective. "
                        "Soft epistemic priors alone do not trigger this path."
                    ),
                    cost_of_rejection="Output delayed for refinement.",
