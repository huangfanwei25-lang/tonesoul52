...
class CouncilTranscript:
    ...
    def to_markdown(self) -> str:
        ...
        for vote in self.votes:
            emoji = {"APPROVE": "✅", "CONCERN": "⚠️", "OBJECT": "❌", "ABSTAIN": "⏸️"}.get(vote.decision, "❓")
            lines.extend([
        ...
        if self.verdict:
            verdict_emoji = {"APPROVE": "✅", "REFINE": "🔄", "DECLARE_STANCE": "⚖️", "BLOCK": "🚫"}.get(self.verdict.decision, "❓")
            lines.extend([
    ...
    def create_transcript(
        ...
        if verdict:
            verdict_record = VerdictRecord(
                decision=str(getattr(verdict, 'decision', getattr(verdict, 'verdict', 'UNKNOWN'))).split('.')[-1],
