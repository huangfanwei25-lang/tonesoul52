...
HYBRID_COUNCIL_CONFIG: Dict[str, Dict[str, str]] = {
    "guardian": {"mode": "llm", "model": "gemini-2.0-flash"},  # Safety: LLM
...
def get_perspective_model(perspective_name: str) -> str:
    ...
    return PERSPECTIVE_MODEL_MAP.get(perspective_name.lower(), "gemini-2.0-flash")
...
def get_council_config(
    ...
    """
    if mode == "full_llm":
        config = dict(MULTI_MODEL_COUNCIL_CONFIG)
    elif mode == "rules_only":
        config = dict(RULES_ONLY_COUNCIL_CONFIG)
    else:
        config = dict(HYBRID_COUNCIL_CONFIG)

    # Apply custom overrides
    if custom_map:
        for perspective, model in custom_map.items():
            if perspective in config:
                config[perspective] = {"mode": "llm", "model": model}
