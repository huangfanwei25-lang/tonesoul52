...
def simulate_meeting():
    ...
    for scenario in scenarios:
        ...
        if triad.delta_t > 0.7:
            print_dialogue(
                "SENSOR", "ALERT: High Tension detected! The user is emotionally volatile."
            )
        ...
        if reasoning_mode.value == "Critical":
            print_dialogue(
                "REASONING",
                "Risk levels are unacceptable. Switching to CRITICAL mode. We need to be careful.",
            )
        elif reasoning_mode.value == "Empathy":
            print_dialogue(
                "REASONING", "User is distressed. Logic is secondary. Switching to EMPATHY mode."
            )
        elif reasoning_mode.value == "Creative":
            print_dialogue(
                "REASONING",
                "Environment is safe and relaxed. Disengaging strict filters. Switching to CREATIVE mode.",
            )
        ...
        if not decision["allowed"]:
            ...
            print_dialogue(
                "GUARDIAN", f"Proposal approved. Safety checks passed. Mode: {decision['mode']}"
            )
