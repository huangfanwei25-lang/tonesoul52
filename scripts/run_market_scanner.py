...
from tonesoul.market.analyzer import SixStepAnalyzer, QuarterlySnapshot, CyclicalTemplate, TechGrowthTemplate
...
def get_mock_watchlist() -> list[StockData]:
    ...
    return [
        ...
        StockData("2330", "TSMC (台積電)", [
            QuarterlySnapshot("2024Q1", 592.6, 0.531, 225.4, 8.70, 225.0, 280.0, 1500.0, 300.0, 430.0, 150.0),
            QuarterlySnapshot("2024Q2", 673.5, 0.532, 247.8, 9.56, 230.0, 300.0, 1550.0, 310.0, 480.0, 200.0),
            QuarterlySnapshot("2024Q3", 759.6, 0.578, 325.2, 12.54, 245.0, 350.0, 1600.0, 350.0, 550.0, 250.0),
            QuarterlySnapshot("2024Q4", 860.0, 0.585, 360.0, 13.88, 250.0, 380.0, 1700.0, 380.0, 600.0, 280.0),
        ]),
        # Innodisk (5289) - Tech Growth 
        StockData("5289", "Innodisk (宜鼎)", [
            QuarterlySnapshot("2025Q1", 26.19, 0.306, 3.32, 3.68, 15.77, 14.87, 28.66, 0, 1.66, 1.07),
            QuarterlySnapshot("2025Q2", 30.28, 0.259, 1.74, 2.02, 26.92, 16.57, 20.68, 2.39, -8.56, -8.50),
            QuarterlySnapshot("2025Q3", 38.07, 0.328, 6.43, 6.87, 28.74, 24.19, 24.95, 1.83, -0.16, -0.44),
            QuarterlySnapshot("2025Q4", 48.07, 0.332, 8.73, 9.29, 53.85, 32.92, 16.45, 13.14, -15.69, -16.12),
        ]),
        # Hon Hai (2317) - Low Margin Tech
        StockData("2317", "Hon Hai (鴻海)", [
            QuarterlySnapshot("2024Q1", 1322.0, 0.063, 22.0, 1.59, 850.0, 1200.0, 1100.0, 1300.0, 15.0, -10.0),
            QuarterlySnapshot("2024Q2", 1550.0, 0.064, 35.0, 2.53, 900.0, 1300.0, 1150.0, 1350.0, 80.0, 40.0),
            QuarterlySnapshot("2024Q3", 1850.0, 0.061, 49.0, 3.55, 1100.0, 1500.0, 1200.0, 1400.0, 150.0, 100.0),
            QuarterlySnapshot("2024Q4", 2100.0, 0.062, 55.0, 3.98, 950.0, 1800.0, 1300.0, 1500.0, 200.0, 150.0),
        ]),
        # Formosa Chem (1326) - Cyclical Decline
        StockData("1326", "Formosa Chem (台化)", [
            ...
            QuarterlySnapshot("2024Q3", 86.2, 0.0211, 0.0, 0.00, 46.5, 34.0, 45.0, 85.0, -3.2, -6.5),
            QuarterlySnapshot("2024Q4", 89.0, 0.0362, -1.1, -0.19, 48.0, 33.0, 40.0, 90.0, -8.5, -12.0),
        ]),
        # Evergreen (2603) - Extreme Cyclical
        StockData("2603", "Evergreen (長榮)", [
            QuarterlySnapshot("2024Q1", 88.6, 0.214, 17.3, 8.14, 4.0, 15.0, 150.0, 45.0, 20.0, 15.0),
            QuarterlySnapshot("2024Q2", 106.2, 0.281, 29.5, 13.7, 4.2, 18.0, 160.0, 40.0, 35.0, 30.0),
            QuarterlySnapshot("2024Q3", 152.0, 0.398, 62.0, 28.7, 4.3, 22.0, 190.0, 35.0, 80.0, 75.0),
            QuarterlySnapshot("2024Q4", 175.0, 0.450, 75.0, 35.0, 4.5, 25.0, 250.0, 30.0, 110.0, 100.0),
        ])
    ]
...
def main():
    ...
    for idx, item in enumerate(watchlist, 1):
        ...
        template = CyclicalTemplate(book_value_per_share=90.0) if industry == "cyclical" else TechGrowthTemplate()
        ...
        if struct_friction <= STRUCTURAL_CUTOFF:
            ...
            print(f"   ↳ ❌ Fail: Structural friction too high (> {STRUCTURAL_CUTOFF}). Skipping AI analysis.")
    ...
    for rank, r in enumerate(results, 1):
        status = "🟢 BUY" if r.final_friction < 0.30 else "🟡 WATCH" if r.final_friction < 0.60 else "🔴 AVOID"
        ai_tag = "[Full AI Verified]" if r.passed_filter else "[Struct Filtered]"
        
        print(f"\n#{rank} | {r.stock.id} {r.stock.name} | {status}")
        print(f"    Total Friction: {r.final_friction:.2f}  (Struct: {r.structural_friction:.2f} | Persona: {r.perspective_friction:.2f})")
