...
def analyze_perspectives(entries):
    """視角一致性分析"""
    perspective_decisions = defaultdict(lambda: {'approve': 0, 'concern': 0, 'object': 0, 'confidences': []})
...
def analyze_divergence_sources(entries):
    ...
    for entry in entries:
        ...
        if core_divergence and core_divergence != 'None':
            # 提取視角名稱
            for perspective in ['Safety Council', 'Analyst Review', 'Critic Lens', 'Advocate Voice']:
...
def generate_insights(entries, verdicts, perspectives, coherence_values):
    ...
    print(f"\n3️⃣ **治理系統穩定運作**")
    print(f"   BLOCK: {block_count}, APPROVE: {approve_count}, DECLARE_STANCE: {declare_stance_count}")
