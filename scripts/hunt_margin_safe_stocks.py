...
def get_margin_watchlist() -> list[MarginStockData]:
    ...
    return [
        MarginStockData("2301", "Lite-On (光寶科)", 105.0, 5.2, [
            QuarterlySnapshot("2024Q1", 28.8, 0.203, 2.4, 1.04, 15.0, 25.0, 50.0, 10.0, 4.5, 3.2),
            QuarterlySnapshot("2024Q2", 33.2, 0.222, 3.1, 1.36, 14.8, 26.0, 52.0, 10.0, 5.0, 4.0),
            QuarterlySnapshot("2024Q3", 36.8, 0.224, 3.5, 1.48, 15.2, 28.0, 55.0, 11.0, 6.2, 5.1),
            QuarterlySnapshot("2024Q4", 39.5, 0.228, 4.0, 1.62, 16.0, 30.0, 58.0, 12.0, 7.0, 5.5),
        ]),
        MarginStockData("4938", "Pegatron (和碩)", 100.5, 4.5, [
            QuarterlySnapshot("2024Q1", 250.0, 0.042, 3.0, 1.1, 100.0, 150.0, 80.0, 60.0, 5.0, -2.0),
            QuarterlySnapshot("2024Q2", 280.0, 0.045, 4.2, 1.5, 105.0, 160.0, 85.0, 65.0, 8.0, 2.0),
            QuarterlySnapshot("2024Q3", 320.0, 0.048, 5.5, 2.0, 120.0, 180.0, 90.0, 70.0, 15.0, 8.0),
            QuarterlySnapshot("2024Q4", 350.0, 0.049, 6.1, 2.2, 125.0, 200.0, 100.0, 75.0, 22.0, 15.0),
        ]),
        MarginStockData("2324", "Compal (仁寶)", 37.8, 4.8, [
            QuarterlySnapshot("2024Q1", 199.0, 0.049, 1.5, 0.35, 70.0, 130.0, 60.0, 45.0, 3.0, 1.0),
            QuarterlySnapshot("2024Q2", 237.0, 0.050, 2.0, 0.45, 75.0, 140.0, 65.0, 40.0, 5.0, 2.5),
            QuarterlySnapshot("2024Q3", 244.0, 0.052, 2.3, 0.52, 78.0, 145.0, 68.0, 40.0, 8.0, 4.0),
            QuarterlySnapshot("2024Q4", 260.0, 0.051, 2.5, 0.58, 80.0, 150.0, 70.0, 40.0, 10.0, 6.0),
        ]),
        MarginStockData("2382", "Quanta (廣達)", 315.0, 3.5, []), # Should be filtered out by price
        MarginStockData("5289", "Innodisk (宜鼎)", 380.0, 2.5, []), # Should be filtered out by price
    ]
...
def main():
    ...
    for item in watchlist:
        ...
        if item.dividend_yield_pct < 4.0:
            print(f"   ❌ Rejected: Yield {item.dividend_yield_pct}% < 4.0% (Risk of interest erosion)")
            continue
            
        print(f"   ✅ Hard Filters Passed: Price {item.current_price} | Yield {item.dividend_yield_pct}%")
