...
def main():
    ...
    snapshots = [
        QuarterlySnapshot(
            quarter="2024Q1",
            revenue=83.9, gross_margin=0.0432, net_income=0.6, eps=0.10,
            inventory=45.0, accounts_receivable=32.0, cash=50.0,
            short_term_debt=80.0, operating_cash_flow=2.5, free_cash_flow=-1.0,
        ),
        QuarterlySnapshot(
            quarter="2024Q2",
            revenue=89.5, gross_margin=0.0585, net_income=0.8, eps=0.15,
            inventory=44.2, accounts_receivable=35.0, cash=48.0,
            short_term_debt=82.0, operating_cash_flow=5.0, free_cash_flow=1.2,
        ),
        QuarterlySnapshot(
            quarter="2024Q3",
            revenue=86.2, gross_margin=0.0211, net_income=0.0, eps=0.00,
            inventory=46.5, accounts_receivable=34.0, cash=45.0,
            short_term_debt=85.0, operating_cash_flow=-3.2, free_cash_flow=-6.5,
        ),
        QuarterlySnapshot(
            quarter="2024Q4",
            revenue=89.0, gross_margin=0.0362, net_income=-1.1, eps=-0.19,
            inventory=48.0, accounts_receivable=33.0, cash=40.0,
            short_term_debt=90.0, operating_cash_flow=-8.5, free_cash_flow=-12.0,
        ),
    ]
    ...
    for sc in scenarios:
        if sc.valuation_model == "PBR":
            print(f"   {sc.name.upper()}: BVPS {sc.bvps_estimate} × PBR {sc.pbr_multiple} = NT${sc.target_price:,.0f} (prob {sc.probability:.0%})")
        else:
