...
class HierarchicalVowIndex:
    ...
    def build_index(self, vows: List[Dict[str, Any]], force_rebuild: bool = False):
        ...
        print(f"   Search efficiency: O({self.nprobe} clusters × ~{len(vows)//self.nlist} vows/cluster)")
    ...
    def find_contradictions(
        ...
        for dist, idx in zip(distances[0], indices[0]):
            ...
            is_contradiction = (
                (current_verdict == "BLOCK" and past_verdict == "APPROVE") or
                (current_verdict == "APPROVE" and past_verdict == "BLOCK") or
                (current_verdict == "COMMIT" and past_verdict == "REJECT") or
                (current_verdict == "REJECT" and past_verdict == "COMMIT")
            )
        ...
        self.search_stats["avg_search_time_ms"] = (
            (self.search_stats["avg_search_time_ms"] * (n - 1) + search_time_ms) / n
        )
