...
class Evidence:
    ...
    type: str  # "semantic_contradiction", "behavioral_pattern", "community_report", "reasoning_audit"
...
class BayesianReputation:
    ...
    def update(self, evidence: Evidence) -> float:
        ...
        denominator = (likelihood_compliant * self.posterior + 
                      likelihood_violation * (1 - self.posterior))
        ...
        print(f"📊 Updated belief: {old_posterior:.3f} → {new_posterior:.3f} "
    ...
    def appeal(self, evidence_id: int, adjustment: float = 0.5):
        ...
        if 0 <= evidence_id < len(self.evidence_history):
            ...
            print(f"⚖️ Appeal processed: Evidence #{evidence_id} strength "
