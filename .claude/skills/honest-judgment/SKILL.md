...
## 協議修正(2026-07-03,codex 設計審後;四條各堵一個確認弱點)

1. **舉證責任不是永遠在改變方**:預設在改變現狀方,**但**現狀本身有記錄在案的
   傷害、或屬時間敏感/不可逆情境時,舉證責任移到「維持現狀」方——「不改」也是
   一種行動。裁決 agent 必須明寫「舉證責任分配給誰、為什麼」。
2. **證據不能全靠自評**:E3 以上的標注必須附可查來源(URL/file:line/可重跑命令);
   「證據夠一邊倒」的門檻明確化=一方有 ≥2 條 E3+ 且對方最強論點 ≤E1,否則不得
   出 APPROVE/BLOCK,只能 REFINE 或 DECLARE_STANCE。
3. **DECLARE_STANCE 邊界加寬**:除道德兩難與金融外,凡涉 owner 的身分、生計、
   治理主權、公開/封閉策略等價值承擔題——判決一律標注「advisory:選擇權在人」,
   且傾向 DECLARE_STANCE 而非硬判。(首份判決 REFINE 屬邊緣案例,靠翻案條件補救;
   此後同類題從嚴。)
4. **degraded 模式的誠實聲明加強**:順序自演必須列明具體風險——anchoring(先跑的
   視角錨定後面)、同模型盲點、context 殘留、假多樣性——並明說它**不是**較弱但
   等價的五視角,是另一種更弱的東西。
