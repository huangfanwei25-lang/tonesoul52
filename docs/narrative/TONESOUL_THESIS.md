...
ToneSoul 的治理建立在 8 條不可變公理上（`AXIOMS.json`），以一階邏輯（FOL）表達：
...
8. **記憶主權** — 記憶轉移與複製必須受 consent 與 owner boundary 約束
...
- 公理 8 → `tonesoul/memory/sovereignty_gate.py` 檢查記憶轉移與複製 consent
