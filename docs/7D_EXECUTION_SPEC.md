...
| RDD | 對抗測試 baseline（至少 10 cases） | `pytest tests/red_team/ -q` | SOFT_FAIL（先） |
| DDD | 討論通道資料完整性 | `python tools/agent_discussion_tool.py audit --path memory/agent_discussion.jsonl` | BLOCKING |
...
2. `RDD`：先採案例數門檻（最小 10 tests），覆蓋率門檻後續再加（已定案）。
