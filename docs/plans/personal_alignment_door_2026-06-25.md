...
- **為什麼**:門的概念連貫(使用者端缺一份**可攜帶、有序、可校正、使用者掌握**的價值/目標 spec)。
  但現在 build 仍撞:公開定位與 auditor handoff 仍在審(#188/#189)、預算有限、倉庫已太大、
  且「門先於房子」——核心 Auditor 雖已 merge(PR #187),但尚未被真實 reviewer/use case 使用。
...
- **不是**私人記憶收集器;不得把個人聊天、個資、API key、商業機密或 private memory 寫進 public repo。
- **不是**runtime prompt auto-injection;任何自動套用到模型上下文的機制都要另過 gate。
...
1. 公開定位 / auditor handoff 佇列清掉(#188/#189 merge 或收斂),管線不壅塞;
2. Fan-Wei × Codex 對接同意這份 spec 的 scope 與 non-goals;
3. 核心(Auditor)已 merge 且至少經過一次真實 reviewer/use case,或明確記錄「未使用仍開工」的理由——
   **房子在,才裝門**;
