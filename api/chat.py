...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            client_id = str(self.client_address[0] if getattr(self, "client_address", None) else "unknown")
            ...
            if error is not None: return send_json_response(self, error[0], error[1])
            ...
            council_mode = _apply_execution_profile_defaults(council_mode, perspective_config, execution_profile)
            message = message if message is not None else ""
            history = history if history is not None else []
            full_analysis = full_analysis if full_analysis is not None else (execution_profile == "engineering")
            ...
            if cached_payload is not None:
                ...
                _persist_chat_side_effects(
                    ...
                    verdict=cached_payload.get("verdict") if isinstance(cached_payload.get("verdict"), dict) else None,
            ...
            traceback.print_exc()
            send_error_response(self, f"Failed to process chat request: {e}", 500)
