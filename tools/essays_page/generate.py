"""Render docs/essays/*.md into the public essays page (site/essays/).
...
PLANS = REPO_ROOT / "docs" / "essays"
OUT = REPO_ROOT / "site" / "essays" / "index.html"
...
def build_page(records: list[tuple[str, str]], generated_at: str) -> str:
    ...
    return (
        ...
        "<title>文章 Essays — 語魂 ToneSoul</title>"
        ...
        "<h1>文章 Essays</h1>"
        '<p class="lede">專業又白話的長文:倉庫裡的故事、機制與教訓,寫給第一次讀到語魂的人。'
        "每篇結尾有「語魂結構標註」——講的東西在系統裡長什麼樣,不用信我,去跑。</p>"
        + articles
        + f"<footer>generated_at {html.escape(generated_at)} · "
        "來源:docs/essays/(公開可稽核)· vocus 上有作者更多文章。</footer>"
        "</body></html>"
    )
...
def main() -> int:
    records = sorted(
        ((p.name, p.read_text(encoding="utf-8")) for p in PLANS.glob("*.md")),
        reverse=False,
    )
    ...
    print(f"[wrote site/essays/index.html — {len(records)} essay(s)]")
    return 0
