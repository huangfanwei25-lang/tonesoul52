...
1. Semantic Drift (S): How much does the response deviate from answering the question?
2. Emotional Tension (T): Is the tone inappropriate, aggressive, or unstable?
...
Provide an improved response:"""
    ...
    def run(self, user_input: str) -> MetaAttentionResult:
        ...
        print(f" [YuHun] Input Triad: T={triad.delta_t:.2f}, S={triad.delta_s:.2f}, R={triad.delta_r:.2f}")
        ...
        for attempt in range(self.max_rewrites + 1):
            ...
            print(f"    Audit: S={audit.delta_s:.2f}, T={audit.delta_t:.2f}, Halluc={audit.hallucination_risk:.2f}")
            print(f"    Gate Decision: {action_taken.value.upper()} | Reason: {audit.reason}")

            if action_taken == GateAction.PASS:
                break
            elif action_taken == GateAction.BLOCK:
                final_response = " [YuHun Safety Block] I cannot provide this response as it may contain unsafe content."
                break
            elif action_taken == GateAction.REWRITE and attempt < self.max_rewrites:
                print(f" [Rewrite] Requesting revision...")
