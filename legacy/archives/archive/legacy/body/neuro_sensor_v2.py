...
# Absolute imports to avoid package-context issues
from body.spine_system import ISensor, ToneSoulTriad
from body.vector_math import Vector, add_vectors, scale_vector, cosine_similarity
    ...
    def _init_anchors(self):
        ...
        def get_axis_vector(positive_concepts, negative_concepts):
            ...
            if not pos_vecs and not neg_vecs:
                print("[WARN] [NeuroSensor] Failed to generate anchors. Using random fallback.")
                return np.random.rand(768) # Should handle better
            
            if not pos_vecs:
                print(f"[WARN] [NeuroSensor] No positive embeddings for axis. Using negative mean for {positive_concepts}.")
                return -np.mean(neg_vecs, axis=0)
            if not neg_vecs:
                print(f"[WARN] [NeuroSensor] No negative embeddings for axis. Using positive mean for {negative_concepts}.")
