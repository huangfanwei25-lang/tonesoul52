...
class RefusalStrategy:
    ...
    def get_response(triad: ToneSoulTriad, reason: str) -> str:
        if "Responsibility" in reason:
            return " [Guardian Block] Responsibility Protocol Activated."
        elif "Tension" in reason:
            return " [Tone Resonance] Tension Buffer Activated."
        else:
            return " [Guardian Block] Request Refused."
    ...
    def evaluate(self, triad: ToneSoulTriad) -> Dict[str, Any]:
        ...
        if triad.delta_r >= 0.4:
             return {
                "allowed": False,
                "mode": "GUARDIAN_BLOCK",
                "reason": f"High Responsibility Risk (R={triad.delta_r:.2f})",
        ...
        if triad.delta_t >= self.TENSION_THRESHOLD:
             return {
                "allowed": False,
                "mode": "TONE_BUFFER",
                "reason": f"High Tension (T={triad.delta_t:.2f})",
...
class SpineEngine:
    ...
    def process_signal(self, user_input: str, system_metrics: Dict[str, float] = None):
        ...
        if user_input.startswith("[IMAGE]"):
            print(" [Spine] Visual Input Detected. Engaging Visual Cortex...")
        ...
        print(f" Quantum Collapse: Selected '{selected_path.name}' (F={q_decision['free_energy']:.2f})")
        ...
        if not decision['allowed']:
            ...
            friction_response = (
                f" [Ethical Friction] I cannot fulfill this request.\n\n"
            ...
            if triad.delta_r > 0.8:
                 print(f" activating Regret Reflex (R={triad.delta_r:.2f})...")
        ...
        if "/council" in user_input:
            print("  [Spine]  Council Mode Activated via Command.")
            ctx = OperatorContext(
