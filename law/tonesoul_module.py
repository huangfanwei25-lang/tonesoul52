...
class ToneSoulModule:
    ...
    def add_to_responsibility_chain(self, actor: str):
        ...
        self._notify_listeners(
            "chain_updated", {"actor": actor, "chain": self.responsibility_chain}
        )
    ...
    def get_vow_history(self, vow_id: str) -> Dict:
        ...
        related_checkpoints = [
            asdict(cp) for cp in self.checkpoints.values() if cp.vow_id == vow_id
        ]
...
if __name__ == "__main__":
    ...
    def log_events(event, data):
        print(
            f"[{event}] {json.dumps(data if isinstance(data, dict) else str(data), ensure_ascii=False, indent=2)}"
        )
